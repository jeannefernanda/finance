...
class Categoria(models.Model):
    ...
    def total_gasto(self):
        ...
        return total_valor
        '''valores = Valores.objects.filter(categoria__id = self.id).filter(data__month=datetime.now().month).aggregate(summarize_address_range('valor'))
    ...
    def calcula_percentual_gasto_por_categoria(self):
        if self.valor_planejamento != 0:
            return (self.total_gasto() * 100) / self.valor_planejamento # calcula a porcentagem
        else:
            return 0
